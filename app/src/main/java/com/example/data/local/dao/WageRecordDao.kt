...
interface WageRecordDao {
    ...
    @Query("SELECT COALESCE(SUM(final_amount), 0.0) FROM wage_records WHERE employee_id = :employeeId")
    suspend fun getTotalWageForEmployee(employeeId: Long): Double

    @Query("SELECT COALESCE(SUM(final_amount), 0.0) FROM wage_records WHERE wage_date >= :startDate AND wage_date <= :endDate")
    suspend fun getTotalWagesInRange(startDate: Long, endDate: Long): Double

    @Query("SELECT COALESCE(SUM(final_amount), 0.0) FROM wage_records")
    suspend fun getTotalWagesAll(): Double
}
