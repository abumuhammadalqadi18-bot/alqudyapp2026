...
class WageAndBalanceTest {
    ...
    fun setup() {
        ...
        reportViewModel = ReportViewModel(employeeRepo, wageRepo, financeRepo, container.settingsRepository, container.companyProfileRepository)
    }
    ...
    fun testTotalNetPayableCalculationAndConstraints() = runBlocking {
        ...
        assertEquals(22000.0, netPayable, 0.01) // 20000 wage + 5000 bonus - 2000 withdrawal - 1000 deduction = 22000.0
    }
    ...
    fun testDayTypeCustom() = runBlocking {
        ...
        val records = wageRepo.getRecordsInRange(0L, 2000L).first { it.isNotEmpty() }
        val record = records.find { it.employeeId == empId }
        requireNotNull(record) { "Record not found! All records: $records" }
